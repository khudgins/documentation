...
which is Enstratius console access and the second is access rights to an individual cloud
resource such as a server.

The elasticity inherent in cloud computing creates a unique challenge for enterprise shell
and remote desktop access. Enstratius operates as an arbiter between your authoritative
...
Enstratius acts as this arbiter by adding individual user accounts to virtual machines
based on the user's Enstratius access rights to that VM. Enstratius maintains a separate set
of login credentials for VM access so that corporate passwords are never placed in a
public cloud.

Any Enstratius user may be granted shell/remote desktop access to a cloud virtual machine
...
(RDP Password) credentials in Enstratius. You can then select the virtual machine to which they
...
authentication credentials (see :ref:`Compute > Servers <saas_servers>` for details). 
...
Users can add these credentials for themselves by editing their profile in the Enstratius
console (see :ref:`Console > Edit Profile <edit_profile>` for details), but administrators can also add them to the
user's account.
...
For details on managing cloud access for users, see :ref:`IAM <saas_iam>`.
...
If you remove the user from Enstratius, Enstratius removes all virtual machine access
as well.
