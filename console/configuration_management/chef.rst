...
There are a few prerequisites required for end-to-end automation using Chef with Enstratius:
...
Before adding your Chef server to the Enstratius console, you'll need two bits of information. This differs based on if you are running OSC vs. OHC/OPC.
...
Additionally, Enstratius will need an API client account and that account's PEM file. This single set of credentials will be used for the following tasks:
...
.. warning:: Enstratius will NEVER alter any records on your Chef server. The logic simply
   doesn't exist in Enstratius.  Enstratius' access is read-only and only to the objects listed
   above. Please see the note below regarding ``admin`` access, however, if you'd like
   Enstratius to take care of a few common termination-related tasks for you.

OSC
~~~

For OSC, you can either provide the existing validation credentials you currently use or
generate new credentials specifically for Enstratius to use.  To generate custom
credentials for Enstratius in OSC, you can use ``knife``:
...
.. note:: Note that we passed the ``-a`` option. This makes the enstratus account an
   administrator. This is optional but **HIGHLY** recommended.  If Enstratius has permissions,
   it will clean up any node and client records for instances Enstratius created. This is very
   valuable in cases where you are relying on search in your recipes as part of an Enstratius
   deployment.  

   **Not only will Enstratius never create any client or node records on its own
   but it will also NEVER delete any client or node records for anything OTHER than instances
   created from within Enstratius**
...
* Provide Enstratius with the original validation cert and user-name or
...
* Create a group that you can easily identify as belonging to Enstratius.
...
"clean up" any instances created by Enstratius. This applies ONLY to instances created from
within Enstratius. If you wish to grant the same permissions to your existing validation
...
Once you've created an account for Enstratius to use and have the credentials, you can add
that to the Enstratius console:
...
At this point, Enstratius will now begin discovery of your ``roles``, ``cookbooks`` and
``environments``.
...
Note that the asynchronous nature ONLY affects the ability of Enstratius to know what objects are available for assignment when launching a server. Enstratius discovers names of cookbooks and the names of the recipes they contain, but does not discover the content. If you make a change to the content of an existing cookbook, recipe or role that Enstratius has discovered, you do not need to wait for Enstratius to rediscover that cookbook, role or recipe. However if you add a NEW recipe to a cookbook, it will not be immediately visible in Enstratius. The same applies for deleting a recipe from a cookbook.
...
.. tip:: When making frequent changes to your Chef cookbooks, it can be difficult to test those
   especially when creating new roles and they have yet to be discovered by Enstratius (and
   ...
   registering the account with Enstratius. Now you can change the run_list of those roles at
   will on the Chef server without Enstratius needing to know anything more than they exist.
...
This script will get information passed to it via the Enstratius agent about your Chef
...
As you can see, Enstratius is standing in for the ``knife bootstrap`` process.

.. note:: Enstratius does not set up any cron jobs or run ``chef-client`` in daemon mode.
   This is a site-specific setting and should be managed in your Chef cookbooks. Enstratius is
...
Enstratius ships "opinionated" scripts but you can customize them as you see fit. Enstratius
only tests with the shipped scripts.

Making an Image available
~~~~~~~~~~~~~~~~~~~~~~~~~~

All interaction with instances from Enstratius is via the agent. Because of
...
Enstratius has been making updated images available with v17 of the agent
installed. You are also free to install the agent yourself.

Regardless of which image you launch (public, Enstratius or preexisting), the image will be
...
with Enstratius. The steps are similar to the previous launch except we're now going to set
...
**ONLY** difference is that the bootstrap is being initiated via the Enstratius agent
instead of via SSH.

Once you click the "Launch Server" button, Enstratius will begin the process of making API
calls to the cloud provider and then waiting for the instance to "phone home".  Once the
server "phones home", Enstratius will make a call back to the agent to begin the
